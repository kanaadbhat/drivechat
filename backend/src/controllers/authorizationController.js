 ...
 */
export const startGoogleDriveAuth = async (req, res) => {
  try {
    const { userId } = req;
    ...
    console.error('❌ [startGoogleDriveAuth] Error:', err.message);
    return res.status(500).json({ error: 'Failed to start authorization', message: err.message });
  }
};
...
export const handleGoogleDriveCallback = async (req, res) => {
  try {
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    return res.redirect(`${frontendUrl}/authorize?error=${encodeURIComponent(err.message)}`);
  }
};
 ...
 */
export const startReauthorization = async (req, res) => {
  try {
    const { userId } = req;
    const userEmail = req.query.email; // Get user's email from Clerk session
    ...
    console.error('❌ [startReauthorization] Error:', err.message);
    return res
      .status(500)
      .json({ error: 'Failed to start re-authorization', message: err.message });
  }
};
 ...
 */
export const handleReauthorizationCallback = async (req, res) => {
  try {
    const { code, state, error } = req.query;
    ...
    console.error('❌ [handleReauthorizationCallback] Error:', err.message);
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    return res.redirect(
      `${frontendUrl}/authorize?error=${encodeURIComponent(err.message)}&reauth=true`
    );
  }
};
